...
# Direct Branch and Bound  uninformed search
def branch_bound_search(problem):
	 return tree_search(problem, BranchBoundQueue())
...
def best_first_graph_search(problem, f):
	...
	With f = node.path_cost there is a branchandboundSearch
	"""
	f = memoize(f, 'f')
...
def branch_bound_subestimation_search(problem, h=None):
	"""Search the nodes with lowest f scores being f(n)=g(n)+h(n)
	g(n) is the path_cost of the node and h(n) is the heuristic.
	We'll use best_first_graph_search function to take the smallest f
	"""
	if h is None:
		h= problem.h
