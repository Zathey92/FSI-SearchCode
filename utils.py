...
class BranchBoundQueue(Queue):
    ...
    def extend(self, items):
		self.A.extend(items) # adds the expanded nodes to the list
		## sort(f(n)) returns an ordered list sorting by the value of f(n) being n a node from the list
		self.A.sort(key=lambda n:n.path_cost) # f(n) is declared as n.path_cost
		
    def pop(self):
		return self.A.pop(0) #returns first node in the list(the one with less path cost)
